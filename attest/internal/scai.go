...
func NewRefValueStatement(subjects []*ita.ResourceDescriptor, attribute string, target *ita.ResourceDescriptor, evidencePath string) (*ita.Statement, error) {
	// generate the resource descriptor for the reference value evidence
	ev, err := generators.NewRdForFile(evidencePath, "", "", "sha256", true, "application/json", "someLocation", nil)
	if err != nil {
